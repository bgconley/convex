...
- **Step 4.4** — Backup & Restore: `backup.sh` (pg_dump custom format + file directory tar → compressed timestamped archive), `restore.sh` (drop/recreate DB, pg_restore, extract files, REINDEX BM25, restart services). Confirmation prompt on restore. **Not yet deployed to GPU server.**
...
### Phase 4 Complete
All implementation plan steps (1.1–4.5) are code-complete.

### Next Step
- Deploy Steps 4.2.1, 4.3, 4.4, 4.5 to GPU server for operational verification
